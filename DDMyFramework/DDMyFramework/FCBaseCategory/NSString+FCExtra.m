...
//  DDMyFramework
//
//  Created by omni－appple on 2019/2/12.
//  Copyright © 2019年 COM.Sobey.dengjie. All rights reserved.
//

#import "NSString+FCExtra.h"
#import <CommonCrypto/CommonCrypto.h>

@implementation NSString (FCExtra)
#pragma mark - 散列函数
- (NSString *)md5String {
...
- (NSString *)hmacMD5StringWithKey:(NSString *)key andSeverTimeWithSever:(NSURL *)url {
    ...
    NSString *hmacKey = [self hmacMD5StringWithKey:md5Key];
    
    NSData *data = [NSData dataWithContentsOfURL:url];
    
    NSDictionary *josn = [NSJSONSerialization JSONObjectWithData:data options:0 error:NULL];
    
    NSString *time = josn[@"time"];
    
    hmacKey = [hmacKey stringByAppendingString:time];
    
    return [hmacKey hmacMD5StringWithKey:md5Key];
}
...
- (NSString *)documentsPath {
    ...
    return [self stringByAppendingString:path.lastPathComponent];
}
